...
body *construct_body_list(int count, double **state){
    ...
    for(long i=0; i<count; i++){
        bodies[i].mass = state[0][i];
        bodies[i].position.x = state[1][i];
        bodies[i].position.y = state[2][i];
    }
...
void print_node(node *N){
    ...
    for(int i=0; i<N->count; i++){
        printf("%lf, %lf, %lf", N->bodies[i].mass, N->bodies[i].position.x, N->bodies[i].position.y);
